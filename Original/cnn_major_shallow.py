...
def getData(filname):
    ...
    X, Y = np.array(X) / 255.0, np.array(Y) # Normalizing X by dividing each pixel value by 255 (Changes range of pixels from 0-255 to 0-1)
    return X, Y

X, Y = getData(filname)
num_class = len(set(Y)) #Stores number of classes in the dataset (i.e 8 (0 to 7))
...
def baseline_model():
    ...
    model.add(Dense(num_class, activation='sigmoid')) # Output layer has 8 neurons (one for each class) (256*8 parameters)
...
def baseline_model_saved(): # Executed if a trained model is saved
    ...
    json_file = open('model_2layer_2_2_pool.json', 'r') # Opens file
    ...
    model.compile(optimizer='adam', loss='binary_crossentropy', metrics=[categorical_accuracy]) # Model is trained with adam optimizer and binary cross entropy loss fuction 
    return model
...
if(is_model_saved==False ): 
    ...
    model.save_weights("model_2layer_2_2_pool.h5") # Saves trained weights
